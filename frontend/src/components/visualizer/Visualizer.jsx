...
function Visualize() {
  ...
  const createNode = (currUser, edgeList1, nodeList1) => {
    ...
    axios.post('http://localhost:8080/getUserFriend', {username : ""+currUser}).then((res) => {
          console.log(res.data);
          return res.data;
          }).then(async input => {
            const promiseArr = [];
            input.forEach((x,i) => {
              const prom = axios.post('http://localhost:8080/getUser', {username : x.friend});
  ...
  useEffect(() => {
    var color = "#BF40BF";
    axios.post('http://localhost:8080/getUserFriend', {username : localStorage.getItem("user")}).then((res) => {
      var nodeList = [];
    ...
    axios.post('http://localhost:8080/getUser', {username : localStorage.getItem("user")}).then((res) => {
