...
function EditAccount() {
  ...
  const updateEmail = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8080/updateEmail', {email: newEmail, username: username});
      setNewEmail('');
    } catch (err) {
      console.error(err);
    }
  ...
  const updateAffiliation = async (e) => {
    ...
    try {
      ...
      console.log(newAff);
      const response = await axios.post('http://localhost:8080/updateAffiliation', {username: username, affiliation: newAff});
    } catch (err) {
      console.error(err);
    }
    try{
      ...
      const response2 = await axios.post('http://localhost:8080/addPost', {author: username, recipient: username, content: status});
    } catch (err) {
      console.error(err);
    }
    setNewAff('');
  ...
  const updatePassword = async (e) => {
    ...
    try {
      ...
      console.log(newPassword);
        const response = await axios.post('http://localhost:8080/updatePassword', {password :newPassword, username : username});
      setNewPassword('');
    } catch (err) {
      console.error(err);
    }
  ...
  const updateInterests = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8080/updateCategories', {username : username, categoryInterest: newInterests});
      }
    } catch (err) {
      console.error(err);
