...
function ProfilePage() {
  ...
  const removeFriend = async (e1) => {
    ...
    axios.post('http://localhost:8080/removeFriend', friendJson).then((res) => {
  ...
  const addFriend = async (e1) => {
    ...
    axios.post('http://localhost:8080/addFriend', friendJson).then((res) => {
          console.log(res.data);
          setIsFriend(true);
          var status = currUsername + " and " + recipient + " are now friends!"
          axios.post('http://localhost:8080/addPost', {author: currUsername, recipient: recipient, content: status});
        }).catch((error) => {
            console.log(error)
        });
  };
  ...
  // Each refresh, check if current profile page user is friends with logged in user & get profile posts if friends
	useEffect(() => {
        ...
        axios.post('http://localhost:8080/getUserFriend', friendJson).then((res) => {
