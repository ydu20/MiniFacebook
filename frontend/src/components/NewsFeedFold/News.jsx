...
function News(props) {
  ...
  useEffect(() => {
    axios.post("http://localhost:8080/getArticleLike", params).then((res) => {
  ...
  function toggleLike () {
    ...
    if (liked) {
      axios.post("http://localhost:8080/deleteArticleLike", params).then((res) => {
        if (!res.data.error) {
          setLiked(false);
        }
      });
    } else {
      axios.post("http://localhost:8080/postArticleLike", params).then((res) => {
        if (!res.data.error) {
          setLiked(true);
        }
      });
